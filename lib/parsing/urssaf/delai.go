...
func (parser *delaiParser) Init(cache *engine.Cache, batch *base.AdminBatch) (err error) {
...
func (parser *delaiParser) ParseLines(parsedLineChan chan engine.ParsedLineResult) {
...
func (parser *delaiParser) parseDelaiLine(row []string, parsedLine *engine.ParsedLineResult) {
	idxRow := parser.idx.IndexRow(row)
	date, err := time.Parse("02/01/2006", idxRow.GetVal("Date_creation"))
	if err != nil {
		parsedLine.AddRegularError(err)
	} else if siret, err := engine.GetSiretFromComptesMapping(idxRow.GetVal("Numero_compte_externe"), &date, parser.comptes); err == nil {
...
func parseDelaiLine(idxRow engine.IndexedRow, siret string, parsedLine *engine.ParsedLineResult) {
	var err error
	delai := Delai{}
	delai.Siret = siret
