...
func (parser *cotisationParser) Init(cache *engine.Cache, batch *base.AdminBatch) (err error) {
...
func (parser *cotisationParser) ParseLines(parsedLineChan chan engine.ParsedLineResult) {
...
func parseCotisationLine(idx engine.ColMapping, row []string, comptes *engine.Comptes, parsedLine *engine.ParsedLineResult) {
	...
	siret, err := engine.GetSiretFromComptesMapping(idxRow.GetVal("Compte"), &periodeDebut, *comptes)
	if err != nil {
