...
echo "${VALIDATION_REPORT}" \
  ...
  | perl -p -e 's/"periode" : ISODate\("....-..-..T..:..:..Z"\)/"periode" : ISODate\("_______ Date _______"\)/' \
  | sort \
  | while read -r line; do ( echo "$line"; DATA_TYPE=$(LINE="$line" node -p 'JSON.parse(process.env.LINE).dataType'); echo "- listing ${DATA_TYPE} validation error(s)..." 1>&2; npx --quiet -p ajv-cli -p ajv-bsontype ajv --strict=false -c "ajv-bsontype" -s "validation/${DATA_TYPE}.schema.json" -d <(echo "$line") || true ); done \
  >> "${OUTPUT_FILE}"
